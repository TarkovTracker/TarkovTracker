...
const useTeamStore = defineStore('team', {
  ...
  getters: {
    ...
    isOwner() { return this.$state?.owner == fireuser.uid || false },
...
watch(teammates, async (newTeammates) => {
    ...
    if (Array.isArray(newTeammates)) {
      for (const teammate of newTeammates) {
        if (!teammateStores.value[teammate]) {
          ...
          teammateStores.value[teammate] = defineStore(`teammate-${teammate}`, {
            state: () => (JSON.parse(JSON.stringify(defaultState))),
            getters: getters,
            actions: actions,
          })
          teammateUnsubscribes.value[teammate] = onSnapshot(doc(firedb, 'progress', teammate), (snapshot) => {
            console.debug(`${teammateStores.value[teammate].$id} data changed`)
            const snapshotData = snapshot.data()
            ...
            if (error.code == 'permission-denied' && teammateUnsubscribes.value[teammate]) {
              console.debug("Unsubscribing from", teammateStores.value[teammate].$id, "due to permission denied")
              teammateUnsubscribes.value[teammate]()
            }
          });
        }
...
const useProgressStore = defineStore('progress', () => {
  ...
  const getDisplayName = function (teamId) {
    return this.teamStores[this.getTeamIndex(teamId)].getDisplayName || teamId.substring(0, 6)
  }
  ...
  return { teamStores, getDisplayName, getTeamIndex, visibleTeamStores, getLevel }
})
