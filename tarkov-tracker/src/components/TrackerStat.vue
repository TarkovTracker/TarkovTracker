<template>
  <v-sheet rounded class="elevation-4 mt-2">
    ...
    <div v-if="$slots.details" class="text-left px-3 py-1 text-grey" style="font-size: .8em">
      <v-divider class="my-1" />
      <v-icon class="mr-1" color="secondary">mdi-help-circle</v-icon><slot name="details"></slot>
    </div>
  </v-sheet>
</template>
