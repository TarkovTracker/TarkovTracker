<template>
  <v-row align="center">
    ...
    <v-col cols="auto">
      <v-btn icon size="x-small" @click="alternativeEditor = !alternativeEditor">
...
const taskAlternatives = computed({
  get() {
    if (editorStore.getAlternativeTasks(props.task.id)?.length > 0) {
      return editorStore.getAlternativeTasks(props.task.id).map(m => tasks.value.find(task => task.id == m))
    } else {
      return []
    }
  },
