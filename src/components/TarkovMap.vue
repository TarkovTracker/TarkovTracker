<template>
  <v-container fluid class="pa-0">
    <v-row v-bind:class="{ 'ma-0': true, 'fullscreen-height': fullscreen, 'normal-height': !fullscreen }">
      <v-col cols="12" md="12" lg="9" xl="8" align-self="center" class="pa-0 my-2">
        <div :id="$id('svgmap')" class="mx-auto pa-0 map-box"
          v-bind:style="{ width: (fullscreen ? mapSize + 'vmin' : mapSize + '%'), position: 'relative' }">
          <div v-for="(objective, objectiveIndex) in mappedObjectives" v-bind:key="`glow-${objectiveIndex}`"
            class="objective-glow" v-show="hoverIndex == objective.id && searchHover == true" :style="{
              position: 'absolute',
          ...
          <v-menu v-for="(objective, objectiveIndex) in mappedObjectives" :key="`menu-${objectiveIndex}`"
            v-model="hover[objective.id]" :close-on-content-click="false" top offset-y auto nudge-top="10"
            transition="scale-transition" v-bind:style="{
              transform: 'translate(-50%, 0%)'
            }">
            <template v-slot:activator="{ on, attrs }">
              <v-icon :id="$id(objective.id)" v-bind="attrs" v-on="on" v-bind:style="{
                position: 'absolute',
                top: objective.gps.topPercent + '%',
                left: objective.gps.leftPercent + '%',
                ...
                @mouseover="hoverIndex = objective.id" @mouseleave="hoverIndex = null">
                {{ hoverIndex == objective.id ? 'mdi-map-marker-radius' : 'mdi-map-marker' }}
              </v-icon>
            </template>
            <v-card class="pa-2" elevation="6">
              <quest-link v-for="(quest, qIndex) in $root.objectiveDictionaryQuests[objective.id].quests"
                :quest-id="quest" :key="`qlink-${objectiveIndex}-${qIndex}`" />
              <quest-objective :quest-objective="objective" :quest-interact="false" :quest-id="objective.quests[0]"
                class="ml-1" />
            </v-card>
          </v-menu>
        </div>
      </v-col>
      <v-col v-if="mapControls == true" class="ma-2" :align-self="fullscreen ? 'center' : 'start'">
        <v-row no-gutters class="my-4">
          <v-slider dense v-model="mapSize" max="100" min="50" label="Scale" color="secondary" class="mr-4"
            hide-details></v-slider>
        </v-row>
        <v-row no-gutters class="my-4">
          <v-col class="text-center">
            <v-tabs v-model="layerSelect" centered background-color="success" color="white" slider-color="white">
              <v-tab v-for="(floor, floorIndex) in floors" :key="`floorTab-${floorIndex}`">
                <v-badge :value="questObjectives.filter(o => hasFloor(o) && o.gps.floor == floor).length > 0"
                  color="primary" :content="questObjectives.filter(o => hasFloor(o) && o.gps.floor == floor).length">
                  {{ floor.replace('_', ' ') }}
                </v-badge>
              </v-tab>
            </v-tabs>
        ...
        <v-row no-gutters class="my-4" v-for="(floor, floorIndex) in floors" :key="`mapfloor-${floorIndex}`"
          v-bind:style="{ opacity: floorIndex == layerSelect ? '1.0' : '.65' }">
          <v-col cols="12" class="text-h5">
            {{ floor.replace('_', ' ') }} Objectives
          </v-col>
          <v-col cols="12" v-if="questObjectives.filter(o => hasFloor(o) && o.gps.floor == floor).length == 0"
            class="text-caption">
            No objectives on this floor
          </v-col>
          <v-col no-gutters @mouseover="hoverIndex = objective.id; searchHover = true;"
            @mouseleave="hoverIndex = null; searchHover = null;" cols="12"
            v-for="(objective, objIndex) in questObjectives.filter(o => hasFloor(o) && o.gps.floor == floor)"
            :key="`floorobjective-${objIndex}`">
            <v-row no-gutters align="start">
              ...
              <v-col align="start">
                <quest-objective :quest-objective="objective" :quest-interact="true" :quest-id="objective.quests[0]" />
              </v-col>
            </v-row>
          </v-col>
        </v-row>
        <v-row v-if="fullscreen && questObjectives.filter(o => !hasFloor(o) && o.type != 'key').length > 0">
          <v-col cols="12" class="text-h5">
            Other Objectives
          </v-col>
          <v-col class="py-0" cols="12"
            v-for="(objective, objIndex) in questObjectives.filter(o => !hasFloor(o) && o.type != 'key')"
            :key="`otherobj-${objIndex}`">
            <v-row no-gutters align="start">
              <v-col cols="auto">
                <quest-objective :quest-objective="objective" :quest-interact="true" :quest-id="objective.quests[0]" />
              </v-col>
