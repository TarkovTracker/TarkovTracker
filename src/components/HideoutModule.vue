<template>
  <material-card :color="moduleColor">
    <template v-slot:heading>
      <img :src="$root.hideoutStationDictionary[moduleDetails.stationId].imgSource" class="pa-2">
    </template>
    ...
    <v-card-text>
      ...
      <template v-if="!modificationDisabled(moduleDetails)">
        <div v-for="requirement in moduleDetails.require" :key="requirement.id" class="text-left"
          :class="haveRequirement(requirement)">
          <span v-if="requirement.type === 'module'">
            <v-icon>mdi-home</v-icon>
          </span>
          <span v-else-if="requirement.type === 'skill'">
            <v-icon>mdi-run</v-icon>
          </span>
          <span v-else-if="requirement.type === 'currency'">
          ...
          <span v-else-if="requirement.type === 'module'">{{ requirement.name }} Level {{ requirement.quantity }}
          </span>
    ...
    <template v-slot:actions>
      <span v-if="pageType === 'available' && !modificationDisabled(moduleDetails)">
        <v-btn class="warning" small @click="clickCompleteModule(moduleDetails)">Build</v-btn>
      </span>
      <span v-else-if="pageType === 'current' && !modificationDisabled(moduleDetails)">
        <v-btn class="error" small @click="clickUncompleteModule(moduleDetails)">Downgrade</v-btn>
      </span>
      <span v-else-if="pageType === 'locked' && !modificationDisabled(moduleDetails)">
        <v-btn class="warning upgrade-hideout" small @click="clickCompleteModule(moduleDetails)">Build anyway</v-btn>
        <v-btn v-for="level in getLockedLevels(moduleDetails)" class="warning upgrade-hideout" small :key="level"
          @click="clickCompleteModule(moduleDetails)">Level {{ level }}</v-btn>
      </span>
    </template>
  </material-card>
</template>
